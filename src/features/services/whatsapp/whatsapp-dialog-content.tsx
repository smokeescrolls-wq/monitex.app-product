...
import { Loader2, MessageCircle, Trash2, Zap } from "lucide-react";
...
import {
  ...
  formatBRPhone,
  normalizePhone,
} from "@/features/services/whatsapp/whatsapp.flow";
...
export default function WhatsappDialogContent({
  serviceKey,
}: {
  ...
  const session = useInvestigationStore((s) => s.sessions.whatsapp);
  ...
  const canStart = useMemo(() => {
    ...
    return phone.length >= 12 && credits >= WA_FLOW.startCost;
  }, [raw, credits]);

  const total = WA_FLOW.steps.length;
  ...
  function onAccelerate() {
    accelerate({ serviceKey: "whatsapp", costPerStep: WA_FLOW.accelerateCost });
  }
          ...
          <CardContent className="p-5 sm:p-6">
            ...
            <div className="mt-4">
              <Input
                ...
                placeholder={WA_FLOW.placeholder}
          ...
          <Card className="border-white/10 bg-black/25 backdrop-blur-xl">
            <CardContent className="p-4">
              ...
              <Button
                ...
                disabled={isCompleted || credits < WA_FLOW.accelerateCost}
                className="mt-3 h-12 w-full rounded-2xl bg-violet-600/80 hover:bg-violet-600 disabled:opacity-50"
              >
                <Zap className="mr-2 h-4 w-4" />
                {credits < WA_FLOW.accelerateCost
                  ? "Créditos insuficientes"
                  : `Acelerar por ${WA_FLOW.accelerateCost} créditos`}
              </Button>
